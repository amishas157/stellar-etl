...
This hypothesis describes the exact same bug as the already-confirmed and published finding in `ai-summary/success/data-transform/001-tx-signers-encode-signature-bytes.md.gh-published`. Both identify that `getTxSigners()` encodes `sig.Signature` (64-byte Ed25519 signature payload) using `strkey.Encode(strkey.VersionByteAccountID, ...)`, producing 108-character fake `G...` account IDs instead of real 56-character signer public keys. The success finding has already been through the full pipeline including PoC verification and adversarial review.

### Code Paths Examined

- `internal/transform/transaction.go:349-357` — `getTxSigners()` helper, same code path identified in success/001

### Why It Failed

This is a duplicate of an already-confirmed and published finding. The success file `001-tx-signers-encode-signature-bytes.md.gh-published` covers the identical mechanism (encoding `sig.Signature` as `AccountID`), the identical affected code paths (`getTxSigners()`, classic and fee-bump branches), and the identical impact (108-character fake account IDs in `tx_signers`).

### Lesson Learned

Check `ai-summary/success/` early in hypothesis generation to avoid re-proposing findings that have already been confirmed and published.
